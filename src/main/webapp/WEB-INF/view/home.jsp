	...
	<div id="wrapper">
		...
		<ul
			...
			<li class="nav-item active"><a class="nav-link"
				href="${pageContext.request.contextPath}/on/calendar"> <i
					class="fas fa-fw fa-tachometer-alt"></i> <span>달력보기</span></a></li>

			<!-- Divider -->
			<hr class="sidebar-divider">

			<!-- Heading -->
			<div class="sidebar-heading">이달의 가계</div>
			...
			<li class="nav-item">
				...
				<div class="bg-white py-2 px-4 collapse-inner rounded">
					...
					<table>
						<tr>
							<td>수입</td>
							<td><c:set var="totalSumIncome" value="0" /> <!-- 수입 총 합을 저장할 변수를 초기화합니다 -->

								<c:forEach var="c" items="${list}">
									<c:if test="${c.category == '수입'}">
										<c:set var="totalSumIncome"
						...
						<tr>
							<td>지출</td>
							<td><c:set var="totalSumExpense" value="0" /> <!-- 지출 총 합을 저장할 변수를 초기화합니다 -->

								<c:forEach var="c" items="${list}">
									<c:if test="${c.category == '지출'}">
										<c:set var="totalSumExpense"
			...
			<!-- Heading -->
			<div class="sidebar-heading">가계부</div>
					...
					<div class="row">
								...
								<div
									...
									<h6 class="m-0 font-weight-bold text-primary">
										...
										<c:forEach var="m" items="${htList}">
											<a
												href="${pageContext.request.contextPath}/on/hashtagList?word=${m.word}"
												style="color: green; text-decoration: none;">${m.word}(${m.cnt})</a>
										</c:forEach>
									</h6>
								</div>
							</div>
							...
							<div class="card shadow mb-4">
								...
								<div
									...
									<div>
										...
										<!--  application.getAttribute("cuurentCounter") -->
									</div>
									<div>
										오늘 접속자 : ${counter}
									...
									<div>
										누적 접속자 : ${totalCounter}
										<!--  request.getAttribute("totalCounter") -->
									</div>
								</div>
							</div>
						</div>
											...
											<c:if test="${!(d<1 || d > lastDate)}">
												<td>
													<div>
														<c:if test="${today == d}">
															<a
																href="${pageContext.request.contextPath}/on/CashbookOne?targetYear=${todayYear}&targetMonth=${todayMonth}&targetDay=${d}"
																style="color: #FFB2D9; text-decoration: none;">${d}</a>
														</c:if>
														<c:if test="${today != d}">
															<a
																href="${pageContext.request.contextPath}/on/CashbookOne?targetYear=${todayYear}&targetMonth=${todayMonth}&targetDay=${d}"
																style="color: #000000; text-decoration: none;">${d}</a>
														</c:if>
													</div>
												</td>
											</c:if>
						...
						<div class="col-xl-4 col-lg-5">
							<div class="card shadow mb-4">
								...
								<div
									class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
									<h6 class="m-0 font-weight-bold text-primary">최근 거래 기록</h6>
									<br>
								</div>
								<br>
